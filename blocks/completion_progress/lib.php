...
function block_completion_progress_bar($activities, $completions, $config, $userid, $courseid, $instance, $simple = false) {
    ...
    $colournames = array(
        ...
        'toImproveRated_colour' => 'toImproveRated_colour',
        'toCheck_colour' => 'toCheck_colour',
        'checkedOk_colour' => 'checkedOk_colour',
        'checkedNo_colour' => 'checkedNo_colour',
    );
    $colours = array();
    ...
    foreach ($activities as $activity) {
        ...
        if ($complete === 'submitted') {
            ...
            if($activity['type'] == 'assign') {
                $assignid = $activity['instance'];
                //$assigngrade = $DB->get_record('assign_grades', ['assignment' => $assignid, 'userid' => $userid]);
                $assigngrades = $DB->get_records('assign_grades', ['assignment' => $assignid, 'userid' => $userid], 'attemptnumber desc', '*', 0, 1);
                $assigngrade = array_shift($assigngrades);

                if($assigngrade && ($assigngrade->grade != -1)){
                    switch($assigngrade->grade) {
                        /*case -1:
                            //$celloptions['style'] .= '#FF0000;'; TODO Czy to jest praca niezaliczona
                            $celloptions['style'] .= $colours['futureNotCompleted_colour'].';';
                            $cellcontent = $OUTPUT->pix_icon('blank', '', 'block_completion_progress');
                            break;*/
                        case 1:
                            // żółty = praca oceniona, wymaga poprawy
                            $celloptions['style'] .= $colours['toImproveRated_colour'].';';
                            $cellcontent = $OUTPUT->pix_icon('blank', '', 'block_completion_progress');
                            break;
                        case 2:
                            // zielony = praca oceniona i zaliczona,
                            $celloptions['style'] .= $colours['checkedOk_colour'].';';
                            $cellcontent = $OUTPUT->pix_icon($useicons == 1 ? 'tick' : 'blank', '', 'block_completion_progress');
                            break;
                    }
                } else if($assignsubmit = $DB->get_record('assign_submission', ['assignment' => $assignid, 'userid' => $userid, 'latest' => '1'])){
                    // fioletowy = praca oddana do sprawdzenia,
                    //'status' => 'submitted'
                    if($assignsubmit->status == 'submitted') {
                        $celloptions['style'] .= $colours['toCheck_colour'] . ';';
                    }
                    // żółty = praca oceniona, wymaga poprawy.
                    //'status' => 'reopened'
                    else if($assignsubmit->status == 'reopened') {
                        $celloptions['style'] .= $colours['toImproveRated_colour'] . ';';
                    }
                    $cellcontent = $OUTPUT->pix_icon('blank', '', 'block_completion_progress');
                }  else {
    ...
    if ($showpercentage == 1 && !$simple) {
        ...
        if ($progress == 100){
        $coursecompletetext = '<i class="fa fa-graduation-cap" aria-hidden="true"></i> <strong>'.get_string('ccp_textline1', 'block_completion_progress').'</strong> <br />'.get_string('ccp_textline2', 'block_completion_progress');
        $coursecompleteclass = array('class' => 'alert alert-success');
        $content .= HTML_WRITER::tag('div', $coursecompletetext, $coursecompleteclass);
        }
    }
    ...
    foreach ($activities as $activity) {
        ...
        if($activity['type'] == 'assign') {
            if(($completed == COMPLETION_COMPLETE) or ($completed == COMPLETION_COMPLETE_PASS)){
                $assignid = $activity['instance'];
                //$assigngrade = $DB->get_record('assign_grades', ['assignment' => $assignid, 'userid' => $userid]);
                $assigngrades = $DB->get_records('assign_grades', ['assignment' => $assignid, 'userid' => $userid], 'attemptnumber desc', '*', 0, 1);
                $assigngrade = array_shift($assigngrades);
                ...
                if($assigngrade && ($assigngrade->grade != -1)){
                //if($assigngrade = $DB->get_record('assign_grades', ['assignment' => $assignid, 'userid' => $userid])){
                    switch($assigngrade->grade) {
                        case -1:
                            //$celloptions['style'] .= '#FF0000;'; TODO Czy to jest praca niezaliczona
                            //$celloptions['style'] .= $colours['futureNotCompleted_colour'].';';
                            $icon = 'blank';
                            //$altattribute = $stringfailed;
                            $altattribute = get_string('toCheck_colour_title', 'block_completion_progress');//'Oddana do sprawdzenia';
                            break;
                        case 1:
                            // żółty = praca oceniona, wymaga poprawy
                            //$celloptions['style'] .= '#FFFF00;';
                            $icon = 'blank';
                            $altattribute = get_string('toImproveRated_colour_title', 'block_completion_progress');//'Oceniona wymaga poprawy';
                            $content .= '- '.get_string('toImproveRated_colour_title', 'block_completion_progress').'&nbsp;';
                            break;
                        case 2:
                            // zielony = praca oceniona i zaliczona,
                            //$celloptions['style'] .= $colours['completed_colour'].';';
                            $icon = 'cross';
                            //$altattribute = $stringpassed;
                            $altattribute = get_string('checkedOk_colour_title', 'block_completion_progress');//'praca oceniona i zaliczona';
                            $content .= '- '.get_string('checkedOk_colour_title', 'block_completion_progress').'&nbsp;';
                            break;
                    }
                } else {
                    if($assignsubmit = $DB->get_record('assign_submission', ['assignment' => $assignid, 'userid' => $userid, 'latest' => '1'])) {
                        //$status = get_string('submissionstatus_' . $assignsubmit->status, 'assign');
                        $icon = 'blank';

                        if($assignsubmit->status == 'submitted') {
                            $altattribute = get_string('toCheck_colour_title', 'block_completion_progress');//'Oddana do sprawdzenia';
                            $content .= '- '.get_string('toCheck_colour_title', 'block_completion_progress').'&nbsp;';// '- Oddana do sprawdzenia&nbsp;';
                        }
                        else if($assignsubmit->status == 'reopened') {
                            $altattribute = get_string('toImproveRated_colour_title', 'block_completion_progress');//'Oceniona wymaga poprawy';
                            $content .= '- '.get_string('toImproveRated_colour_title', 'block_completion_progress').'&nbsp;';// - Praca oceniona, wymaga poprawy.
                        }
                    }
                }
            }
