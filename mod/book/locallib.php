...
function book_get_toc($chapters, $chapter, $book, $cm, $edit) {
    ...
    if ($edit) { // Editing on (Teacher's TOC).
        ...
        $toc .= html_writer::start_tag('ul', array('class' => 'spoledtoc'));
            ...
            if (!$ch->hidden) {
                if (!$ch->subchapter) {
                    ...
                    if ($first) {
                        ...
                        $toc .= html_writer::end_tag('ul', array('class' => 'end'));
                        $toc .= html_writer::end_tag('li');
                        $toc .= html_writer::start_tag('li', array('class' => 'clearfix'));
                    }
                    ...
                    if(!empty($ch->subchapters)){
                        ...
                        if (in_array($chapter->id, $ch->subchapters)) {
                            $ariaExpanded = 'true';
                            $icon = 'up';
                        ...
                        $attributes = ["title" => s($titleunescaped)];
                        if ($ch->id == $chapter->id) {
                            $attributes["style"] =  'font-weight: bolder;';
                        }

                        $attributesIcon = [
                            "aria-hidden" => "true",
                            "class" => "more-less fa fa-angle-".$icon,
                        ];

                        $attributesToogle = [
                            "class" => "coursetoc-header",
                            "type" => "a",
                        ...
                        $icon = html_writer::tag('i', '', $attributesIcon);
                        $linkToggle = html_writer::link("#", ' '.$icon, $attributesToogle);
                        $linkToggle = '';

                        $title = html_writer::link(new moodle_url('view.php',
                            array('id' => $cm->id, 'chapterid' => $ch->id)),
                            $title , $attributes) . $linkToggle;



                        /*$title = '<a href="#" class="coursetoc-header" type="a"
                                            data-toggle="collapse"
                                            data-target="#collapse'.$ch->id.'"
                                            aria-expanded="'.$ariaExpanded.'"
                                            aria-controls="collapse'.$ch->id.'">'.

                        $title.' <i class="more-less fa fa-angle-'.$icon.'" aria-hidden="true"></i>---</a>';*/
                ...
                if ($ch->id == $chapter->id) {
                    ...
                    $toc .= html_writer::link("#", $title,
                        array('title' => s($titleunescaped), 'class' => $cssclass, 'style' => 'font-weight: bolder;'));
                } else {
                ...
                if (!$ch->subchapter) {
                    ...
                    $toc .= html_writer::start_tag('ul', array('id' => "collapse{$ch->id}", 'class' => $classShow));
                } else {
                    $toc .= html_writer::end_tag('li');
                }
