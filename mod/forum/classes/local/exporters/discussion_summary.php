...
class discussion_summary extends exporter {
    ...
    protected static function define_other_properties() {
        return [
            'id' => ['type' => PARAM_INT],
            'userfirst_course_role' => ['type' => PARAM_TEXT],
            'userlast_course_role' => ['type' => PARAM_TEXT],
            'discussion' => [
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $firstusd =$firstpostauthor->export($output)->id;

        $coursecontext = \context_course::instance($forum->get_course_id());
        $rolesfirstpost = get_user_roles($coursecontext, $firstpostauthor->export($output)->id);
        $roleslastpost = get_user_roles($coursecontext, $latestpostauthor->export($output)->id);

        if(!empty($rolesfirstpost)){
            foreach($rolesfirstpost as $item) {
                if(isset($item->shortname)) {
                    $userfirst_course_role =$item->shortname;
                }else{
                    $userfirst_course_role = null;
                }
            }
        } else{
            $userfirst_course_role = null;
        ...
        if(!empty($rolesfirstpost)){
            foreach($roleslastpost as $item) {
                if(isset($item->shortname)) {
                    $userlast_course_role =$item->shortname;
                }else{
                    $userlast_course_role = null;
                }
            }
        } else{
            $userlast_course_role = null;
        ...
        return [
            ...
            'userfirst_course_role' => $userfirst_course_role,
            'userlast_course_role' => $userlast_course_role,
