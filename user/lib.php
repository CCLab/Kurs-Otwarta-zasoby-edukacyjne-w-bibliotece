...
function user_get_default_fields() {
    ...
    $fields = array( 'id', 'username', 'fullname', 'firstname', 'lastname', 'email',
...
function user_get_participants_sql($courseid, $groupid = 0, $accesssince = 0, $roleid = 0, $enrolid = 0, $statusid = -1,
    ...
    if (!empty($search)) {
        ...
        foreach ($search as $index => $keyword) {
            ...
            if (is_siteadmin($USER)) {  // Only admins may edit other admins.
                $identityfields = explode(',', $CFG->showuseridentity);
                $identityfields[] = 'idnumber';
                $CFG->showuseridentity = implode(',', $identityfields);
            }
