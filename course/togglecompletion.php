...
if ($courseid) {
    ...
    if ($user && $rolec) {
        ...
        if ($confirm and confirm_sesskey()) {
            ...
            if ($from == 'report') {
                redirect($CFG->wwwroot . '/report/completion/index.php?course=' . $courseid);
            }
        ...
        $userconfirm = $DB->get_record('user', ['id' => $user]);

        $strconfirm = get_string('confirmselfcompletion', 'theme_spoled');
        $strconfirm .= ' ('.$userconfirm->firstname . ' ' .$userconfirm->lastname .')';
        ...
        $buttoncontinue = new single_button(new moodle_url('/course/togglecompletion.php',
            array(
                ...
                'from' => $from,
                'confirm'=>1,
                'sesskey'=>sesskey())), get_string('yes'), 'post');

        // Return to previous page
        $referer = get_local_referer(false);
        if (!empty($referer)) {
            //redirect($referer);
            $buttoncancel   = new single_button(new moodle_url($referer), get_string('no'), 'get');
        } else {
            redirect('view.php?id=' . $course->id);
            $buttoncancel   = new single_button(new moodle_url('/course/view.php', array('id'=>$course->id)), get_string('no'), 'get');
        }
