...
class filter_filtercodes extends moodle_text_filter {
    ...
    public function filter($text, array $options = []) {
        global $CFG, $SITE, $PAGE, $USER, $DB, $COURSE;
        ...
        if (strpos($text, '{if') !== false) { // If there are conditional tags.
            ...
            // Tag: {ifgroup-"idgroup"} wyświetla jeśli użytkownik kursu jest przypisany do jakieś grupy w kursie
            // ale musi byc zalogowany, nie być gościem oraz musi być w jakieś grupie danego kursu
            // Przykłąd: Kursant nalezy do grupy o id 1 {ifgroup-1}Ta treść zostanie wyświetlona{/ifgroup-1}
            if (isloggedin() && !isguestuser()) {

                //Pobranie grup użytkownika dla danego kursu
                $mygroups = groups_get_all_groups($COURSE->id, $USER->id);

                //Jeśli nie jest w żadnej grupie usuń wszystkie tagi ifgroup
                if (empty($mygroups)) {
                    if (stripos($text, '{ifgroup-') !== false) {
                        $replace['/\{ifgroup-\d*\}(.*?)\{\/ifgroup-\d*\}/ims'] = '';
                    }
                }else{

                    //Wypisanie id grup w kursie do których użytkownik jest zapisany
                    //i wyświetlenie treści tagów .
                    $item = null;
                    foreach($mygroups as $struct) {

                        $v = $struct->id;

                        if (stripos($text, '{ifgroup-'.strval($v).'}') !== false) {

                            // Usunięcie znaczników tagów
                            $replace['/\{ifgroup-'.strval($v).'\}/i'] = '';
                            $replace['/\{\/ifgroup-'.strval($v).'\}/i'] = '';
                            }

                    }
                    //Jeśli zostały jakieś tagi do których użytkownik nie ma dostępu
                    // zostają usunięte.
                    if (stripos($text, '{ifgroup-') !== false) {
                        $replace['/\{ifgroup-\d*\}(.*?)\{\/ifgroup-\d*\}/ims'] = '';
                    }
                ...
                // Jeśli użytkownik nie jest zalogowany bądź jest gościem usuń wszytkie tagi ifgroup
                if (stripos($text, '{ifgroup-') !== false) {
                    $replace['/\{ifgroup-\d*\}(.*?)\{\/ifgroup-\d*\}/ims'] = '';
                }
            }
